...
### ・不具合/修正
* <s>不具合: network状態は良好で、検索時に正常に取得しているが、Resultコンポーネントで更新を補足していない</s>
* 不具合: fetchが発火していなかった。useSearchが正常に使われているか確認する
* 確認: network上を確認すると、ボタンクリックとともにページが初期化されている
* 解決: event発生を制御して解消
* 不具合: 検索がヒットしなかった場合エラーを吐く仕様になっているため、要修正
...
## 不具合
* APIから エラー:不正な検索条件 を受け取る
* 差分確認: 異常変更なし。Form完成時のコード編集に問題ありと予想
* 原因特定: 検索結果がないときに、型に違反する
## 修正
* バリデーションで schema.pick() を使って空レコードが返らないことを明示

## ver.1 完成
* 日付入力の補助や詳細オプションの追加などの機能追加が可能だがここで一区切りとする
* ドメイン・責務分離の演習として完成しているため、機能追加は後に委ねる
